...
export class PinsModule {
    ...
    async update() {
        await this.load();
        // Если на дашборде — обновляем отображение
    ...
    groupPins() {
        ...
        this.pins.forEach(pin => {
            ...
            if (this.groups[type]) {
                ...
                this.groups.sensor.pins.push(pin);
            }
        });
    }
    ...
    render() {
        ...
        const active = this.pins.filter(p => p.online).length;
        ...
        html += `
            <div class="stats-bar">
                <div class="stat-item">Всего пинов: <span id="totalPins">${total}</span></div>
                <div class="stat-item">Датчики: <span id="sensorCount">${sensors}</span></div>
                <div class="stat-item">Выходы: <span id="actuatorCount">${actuators}</span></div>
                <div class="stat-item">Входы: <span id="inputCount">${inputs}</span></div>
        ...
        Object.keys(this.groups).forEach(groupKey => {
            ...
            group.pins.forEach(pin => {
                const isActuator = pin.type === 'actuator' || pin.is_output === true;
                ...
                if (isActuator) {
                    ...
                    controls = `
                        <button class="${isOn ? 'btn-off' : 'btn-on'}"
                                onclick="window.app.modules.pins.setPin(${pin.id}, ${isOn ? 0 : 1})">
                            ${isOn ? '🔴 Выкл' : '🟢 Вкл'}
                        </button>
                    `;
        ...
        // Обновляем счётчик включенных выходов
        setTimeout(() => {
    ...
    updateStats() {
        // Статистика уже обновляется в render
    }
